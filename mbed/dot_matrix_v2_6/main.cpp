...
//
void send_nextMatrix( string sNo){
    int iCt=0;
    int iSpan= 6;
    string sBuff="";
    int iLen= mReceive2.length();
    mPc.printf("%s", sNo);
    while(1){
        sBuff=mReceive2.substr(iCt);
        if(sBuff.length() >= iSpan){
            sBuff=mReceive2.substr(iCt, iSpan);
            mPc.printf("%s" ,sBuff);
            wait_ms(100);
            iCt=iCt+iSpan;
        }else{
